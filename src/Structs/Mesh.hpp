...
public:
	...
	struct Face {
		...
		bool isDisabled() const { return m_he == std::numeric_limits<IndexType>::max(); }
		...
		uint8_t			m_horizonEdgesOnCurrentIteration : 3; // Bit for each half edge assigned to this face, each being 0 or 1 depending on whether the edge belongs to horizon edge
...
template <typename T>
IndexType MeshBuilder<T>::addFace()
{
...
template <typename T>
IndexType MeshBuilder<T>::addHalfEdge()
{
...
template <typename T>
std::unique_ptr<std::vector<IndexType>> MeshBuilder<T>::disableFace(IndexType faceIndex)
{
...
template <typename T>
void MeshBuilder<T>::disableHalfEdge(IndexType heIndex)
{
...
template <typename T>
MeshBuilder<T>::MeshBuilder(IndexType a, IndexType b, IndexType c, IndexType d)
{
	// Create halfedges
	HalfEdge AB;
	AB.m_endVertex = b;
...
template <typename T>
std::array<IndexType, 3> MeshBuilder<T>::getVertexIndicesOfFace(const Face& f) const
{
...
template <typename T>
std::array<IndexType, 2> MeshBuilder<T>::getVertexIndicesOfHalfEdge(const HalfEdge& he) const
{
	return {m_halfEdges[he.m_opp].m_endVertex,he.m_endVertex};
}

template <typename T>
std::array<IndexType, 3> MeshBuilder<T>::getHalfEdgeIndicesOfFace(const Face& f) const
{
